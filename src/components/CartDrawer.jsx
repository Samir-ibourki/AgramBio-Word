...
function CartDrawer({ isOpen, onClose }) {
  ...
  const { cart, removeFromCart, updateQuantity, getSubtotal, shippingFee, getTotal } = useCartStore();
  ...
  return (
    <>
      ...
      {/* drawer */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-[210] shadow-2xl transition-transform duration-500 transform ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-b border-black/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-serif text-dark font-bold">{t('cart.title')}</h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                ...
                <button onClick={onClose} className="mt-6 text-xs uppercase font-bold tracking-widest text-gold hover:underline">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                      <div className="flex justify-between items-start">
                        <h3 className="text-sm font-bold text-dark">{getName(item.name)}</h3>
                        <button onClick={() => removeFromCart(item.id)} className="text-dark/20 hover:text-red-500 transition-colors">
                          <Trash2 size={14} />
                        </button>
                      </div>
                      <p className="text-[10px] text-dark/40 uppercase tracking-widest mt-1">{item.categoryLabel}</p>
                    </div>
                    
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3 border border-black/5 rounded-lg px-2 py-1">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="text-dark/40 hover:text-gold"><Minus size={12} /></button>
                        <span className="text-xs font-bold w-4 text-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="text-dark/40 hover:text-gold"><Plus size={12} /></button>
                      </div>
                      <p className="text-sm font-bold text-gold">{item.price * item.quantity} MAD</p>
                    </div>
                  </div>
                </div>
              ))
