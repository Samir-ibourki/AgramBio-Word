  ...
  const getName = useCallback(
    (name) => {
      if (typeof name === "object")
        return name[lang] || name.fr || name.ar || "";
      return name || "";
    },
    [lang],
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "price-high":
        ...
        break;
      case "name":
        result.sort((a, b) => getName(a.name).localeCompare(getName(b.name)));
        break;
      default:
  ...
  return (
    <div className="min-h-screen bg-[#FCFAFA] pb-24 pt-10">
      <Helmet>
        ...
        <meta
          name="description"
          content="Explorez notre catalogue complet de produits naturels marocains. Miel, huiles végétales et Amlou d'exception."
        />
      </Helmet>
      <div className="max-w-7xl lg:max-w-[95vw] mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="relative w-full md:w-80 group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-dark/20 group-focus-within:text-gold transition-colors"
              size={18}
            />
            <input
              type="text"
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          <aside className="w-full lg:w-64 space-y-10">
            <div className="p-8 bg-white border border-black/5 rounded-3xl">
              <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-dark mb-6">
                {t("shop.categories")}
              </h3>
                ...
                {categories.map((cat) => (
                  ...
                  >
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 peer-checked:opacity-100 text-white text-[10px]">
                        ✓
                      </div>
                    </div>
                    <span
                      className={`text-sm tracking-wide transition-colors ${selectedCategories.includes(cat.slug) ? "text-dark font-bold" : "text-dark/40 group-hover:text-dark"}`}
                    >
                      {t(cat.labelKey)}
                    </span>
                  </label>
            ...
            <div className="p-8 bg-white border border-black/5 rounded-3xl">
              ...
              <div className="flex justify-between mt-4">
                <span className="text-[10px] text-dark/30 font-bold uppercase">
                  0 MAD
                </span>
          ...
          <main className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              ...
              <div className="relative group">
                ...
                >
                  {t("shop.sort_by")}{" "}
                  <span className="text-dark">{t(currentSortLabel)}</span>
                  <ChevronDown
                    size={14}
